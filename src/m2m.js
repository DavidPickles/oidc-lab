...
async function run() {
    ...
    const outgoingRequestOpts = getOutgoingRequestOpts({ httpsSecurity: process.env.OUTGOING_HTTPS_SECURITY })
    ...
    if (namedApiEndpoints.length) {
        ...
        const endpointResps = await apis.getResponses(namedApiEndpoints, token, outgoingRequestOpts)
