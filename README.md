...
OUTGOING_HTTP_SECURITY | Optional | If "none", potential security issues with requests to https: endpoints on the OP and on any API requests will be ignored, the requests are thereby made insecure. (The axios agent options this sets are rejectUnauthorized: false, and secureOptions: crypto.constants.SSL_OP_LEGACY_SERVER_CONNECT)
