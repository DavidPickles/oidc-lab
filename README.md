...
OUTGOING_HTTPS_SECURITY | Optional | Set to  "none" to ignore potential security issues with outgoing requests to https endpoints including requests to the authorization server and to any API endpoints. The requests are thereby made insecure. Sometimes the network you're on, or limitations of your development authorization server make this unavoidable. <br><br>If you need to do this, you will also need to use IDP_BASE_URL instead of ISSUER. With ISSUER OIDC Lab uses a library to obtain keys to verify tokens and that will be affected by https issues; but the library can't be configured to ignored those issues, so you have dispense with verification altogether.   
HOME_PATH | Optional | By default the home URL of the app is BASE_URL, this parameter can override it
IDP_BASE_URL | Required if no ISSUER | Base URL of the authorization server. Providing this instead of ISSUER means that tokens will not be verified. 
